...
## Watch-Command
##
## Test du celebrissime Nicolas Fazilleau
##
########################################################################
...
Wathes the result of a command incocation, alerting tou when the output eithher matches a specified string, lacks a specified string, or has simply changed

.EXAMPLE
PS > Watch-command {Get-Process -Name Notepad | Measure} -UntilChanged
Monitors Notepad process until you start or stop one.

.EXAMPLE 
PS > Watch-command {Get-Process -Name Notepad | Measure} -Until "Count     :1"
Monitors Notepad process until there is exactly one open.
...
param(
    ...
    ## Monitoring continues only while the output of the command 
    ## remains the same
    [Parameter(ParameterSetName = "UntilChanged",Mandatory = $false)]
    [Switch] $UntilChanged,

    ## The regular Ewpression to search for . Monitoring continues
    ## until the expression is found.
    [Parameter(ParameterSetName = "Until",Mandatory = $false)]
    [String] $Until,

    ## The regular expression to search for. Monitoring continues
    ## th expression is not found
